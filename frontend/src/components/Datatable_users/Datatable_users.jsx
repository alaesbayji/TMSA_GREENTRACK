...
const Datatable_users = () => {
  ...
  const handleDelete = async (userData) => {
    ...
    try {
      await api.delete(`http://localhost:8000/api/responsables-entreprise/${userData.id}/delete/`);
  ...
  const userColumns = [
    ...
    { field: 'zoneName', headerName: 'Zone', width: 150 }, // Ajouter la colonne pour le nom de la zone
  ];
  ...
  return (
    <div className="datatable">
      <div className="datatableTitle" style={{ color: '#00A2DD' }}>
        Gestion des Responsables d'Entreprise
        <Link to="/ajouter_responsable_entreprise" className="link">
          <PersonAddIcon/> Ajouter un nouveau responsable
        </Link>
      </div>
      <DataGrid
        className="datagrid"
