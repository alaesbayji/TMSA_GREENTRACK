...
const AjoutIndicateurSAForm = () => {
  ...
  useEffect(() => {
    const fetchSousAspects = async () => {
      try {
        ...
        const filteredAspects = response.data.filter(aspect => aspect.est_pollution === false);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:8000/api/indicateurs-sous-aspect/?id_sous_aspect=${sousAspectId}`);
    ...
    try {
      ...
      if (currentIndicateur) {
        response = await axios.put(
          `http://localhost:8000/api/indicateurs-sous-aspect/${currentIndicateur.id_indicateur_sous_aspect}/`,
          indicateurData
        );
      } else {
        response = await axios.post('http://localhost:8000/api/indicateurs-sous-aspect/', indicateurData);
      }

      const newIndicateur = {
        id_indicateur_sous_aspect: response.data.id_indicateur_sous_aspect,
      ...
      if (currentIndicateur) {
        setIndicateurs((prev) =>
          prev.map((i) => (i.id_indicateur_sous_aspect === newIndicateur.id_indicateur_sous_aspect ? newIndicateur : i))
        );
      } else {
  ...
  const deleteIndicateur = async (id_indicateur_sous_aspect) => {
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer cet indicateur ${id_indicateur_sous_aspect}?`)) {
      try {
        await axios.delete(`http://localhost:8000/api/indicateurs-sous-aspect/${id_indicateur_sous_aspect}/`);
        setIndicateurs((prev) => prev.filter((i) => i.id_indicateur_sous_aspect !== id_indicateur_sous_aspect));
  ...
  return (
    <div className="formSection">
      <h2>Gestion des Indicateurs pour le Sous-Aspect</h2>
      ...
      {sousAspectId && (
        <>
          ...
          <Paper sx={{ marginTop: '10px', height: 'auto', width: '100%', '& .MuiDataGrid-columnHeader': { backgroundColor: '#f9f9f9' } }}>
            <DataGrid
              rows={indicateurs.map((indicateur) => ({ ...indicateur, id: indicateur.id_indicateur_sous_aspect }))}
...
export default AjoutIndicateurSAForm;